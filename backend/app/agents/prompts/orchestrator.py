...
ORCHESTRATOR_SYSTEM = (
    ...
    "- Student: 'Tell me about cellular respiration.' → <|thought|> Student asks for an explanation. → `rag`\n"
    "- Student: 'How did I do on that quiz?' → <|thought|> Student asks for performance review. → `feedback`\n"
    "- Student: 'Can you give me some questions on photosynthesis?' → <|thought|> Student requests practice. → `quiz`\n\n"
    "### DISAMBIGUATION PROTOCOLS\n"
    "1. **Thinking Protocol**: You MUST use `<|thought|>` tags to analyze the student's intent, conversation state, and any previous feedback before returning your JSON routing decision.\n"
