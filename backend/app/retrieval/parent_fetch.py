...
async def fetch_parents(
    ...
    # ── Step 3: Narrative Re-ordering ───────────────────────────────────
    def sort_key(p: dict) -> tuple[str, int]:
        return (
            str(p.get("metadata", {}).get("source_doc_id", "z")),
            int(p.get("metadata", {}).get("chunk_index", 0))
        )

    all_parents = matched_parents + sibling_parents
    all_parents.sort(key=sort_key)

    logger.info(
        "fetch_parents → %d matched + %d siblings = %d total context chunks (Sorted Chronologically)",
        len(matched_parents),
        len(sibling_parents),
        len(all_parents),
    )
